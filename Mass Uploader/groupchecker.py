...
def getcursordata(groupId, cursor):
    itemresponse = requests.get(f"https://itemconfiguration.roblox.com/v1/creations/get-assets?assetType=Image&groupId={groupId}&isArchived=false&limit=100&cursor={cursor}", headers={"Accept": "application/json", "Content-Type": "application/json"}, cookies={".ROBLOSECURITY": cookie})
    if itemresponse.status_code == 200:
        return itemresponse.json()
    print(f"Something went wrong (Retrying). Status Code: {itemresponse.status_code}")
    time.sleep(2.5)
    return getcursordata(groupId, cursor)
...
def saveToFile(content, groupId):
    if not os.path.exists("output"):
        os.makedirs("output")
    with open(os.path.join("output", f"groups_accepted_{groupId}.txt"), 'a') as file:
        file.write(content + '\n')
...
if __name__ == "__main__":
    ...
    while True:
        page += 1
        data = getcursordata(groupId, cursor)
    ...
    if not os.path.exists("output"):
        os.makedirs("output")
    if not os.path.exists(os.path.join("output", f"groups_accepted_{groupId}.txt")):
        with open(os.path.join("output", f"groups_accepted_{groupId}.txt"), 'w') as file:
            file.write("Accepted decals from a group listed below.\n")
    csrfrecheck = 0
    ...
    for assets in chunks:
        ...
        for asset in response:
            if asset["status"] == "ReviewApproved":
                ...
                saveToFile(f"https://www.roblox.com/library/{asset["assetId"]}", groupId)
        csrfrecheck += 1
        time.sleep(0.75)
